            ...
            <form method="POST" action="{{ route("admin.doctors.update", [$doctor->id]) }}"
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name"
                           id="name" value="{{ old('name', $doctor->name) }}">
                    @if($errors->has('name'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email"
                           name="email" id="email" value="{{ old('email', $doctor->email) }}">
                    @if($errors->has('email'))
                        <div class="invalid-feedback">
                            {{ $errors->first('email') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    {{-- <input class="form-control {{ $errors->has('bio') ? 'is-invalid' : '' }}" type="text" name="bio" id="bio" value="{{ old('bio', $doctor->bio) }}"> --}}
                    <textarea class="form-control {{ $errors->has('bio') ? 'is-invalid' : '' }}" name="bio"
                              id="bio">{{ old('bio', $doctor->bio) }}</textarea>
                    @if($errors->has('bio'))
                        <div class="invalid-feedback">
                            {{ $errors->first('bio') }}
                        </div>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('is_first_period') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="is_first_period" id="is_first_period"
                               value="1" {{ $doctor->is_first_period == 1 ? 'checked' : '' }}>
                ...
                <div class="form-group">
                    <div class="form-check {{ $errors->has('is_second_period') ? 'is-invalid' : '' }}">
                        ...
                        <input class="form-check-input" type="checkbox" name="is_second_period" id="is_second_period"
                               value="1" {{ $doctor->is_second_period == 1 ? 'checked' : '' }}>
                        <label class="form-check-label"
                ...
                <div class="form-group">
                    <div class="">
                        ...
                        <input class="form-control date {{ $errors->has('start') ? 'is-invalid' : '' }}" type="text"
                               name="start" id="start" value="{{ old('start', $doctor->start) }}">
                        @if($errors->has('start'))
                            <div class="invalid-feedback">
                                {{ $errors->first('start') }}
                            </div>
                    ...
                    <div class="">
                        ...
                        <input class="form-control date {{ $errors->has('end') ? 'is-invalid' : '' }}" type="text"
                               name="end" id="end" value="{{ old('end', $doctor->end) }}">
                        @if($errors->has('end'))
                            <div class="invalid-feedback">
                                {{ $errors->first('end') }}
                            </div>
...
@section('scripts')
    <script>
        Dropzone.options.pictureDropzone = {
            ...
            init: function () {
                @if(isset($config) && $config->picture)
                var file = {!! json_encode($config->picture) !!}
